...
5. 以下のコマンドを実行し、1で切り替えた環境の 
　 環境変数（./.azure/{env_name}/.env ファイルの設定内容）が表示されることを確認する。
azd env get-values
　 別の環境の環境変数が表示されたり、何も表示されなかったりする場合は、VSCodeのターミナルではなく、OS上から新しい PowerShell ウィンドウを開き、そちらで試す。
　 その際、最初にプロジェクトルートフォルダーへ移動する事を忘れないこと（cd {prj_root}）
